...
public:
	...
	SavingsAccount(int NewID, double NewBalance, double NewRare,int NewDate); //内部构造函数 参数 date; ID; rate
	...
	void show()const { cout << ID<<" "<<balance<<" "<<rate<<" "<<date<<endl; } //展示 int ID, double Balance, double Rate, int Date

	void deposit(int Newdate = 0, double amount=0);
	void withdraw() {}
	void settle() {} //利息结算
	//
...
SavingsAccount::SavingsAccount(int NewID, double NewBalance, double NewRate, int NewDate): ID(NewID), balance(NewBalance),rate(NewRate),date(NewDate),accumulation(0){
	ID = NewID;
	balance = NewBalance;
	rate = NewRate;
	date = NewDate;
...
void BankAccount() {
	SavingsAccount account1(10000, 500, 10,10); //int NewID, double NewBalance, double NewRate, int NewDate
	account1.show();
	account1.deposit(3, -54);
	account1.show();
}
